#!/usr/bin/env python3
"""Run G9b blood eQTL enrichment AFTER freeze hash exists.

HTTP 400 → MISS. Primary QTD000356. Replication QTD000373 only if PASS.
"""
...
RESULT = OUT / "g9b_common_alu_ctcf_blood_eqtl_v1.json"
DECISION = OUT / "G9b_common_alu_ctcf_blood_eqtl_decision_v1.md"
PROGRESS = OUT / "g9b_eqtl_progress.txt"
...
def _write_decision(payload: dict) -> None:
    ...
    if payload.get("replication"):
        ...
        lines += [
            ...
            "",
            f"- verdict: {r['decision']['verdict']} ({r['decision'].get('reason', '')})",
            f"- p_case={r['decision'].get('p_case')} p_ctrl={r['decision'].get('p_ctrl')} "
            f"fisher_p={r['decision'].get('fisher_p')}",
            "",
        ]
    DECISION.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> int:
    ...
    if not FREEZE.exists() or not FREEZE_SHA.exists():
        raise SystemExit("freeze artifacts missing — run freeze_g9b first")
    ...
    if len(cases) < 30 or len(ctrls) < 30:
        ...
        payload = {
            "result_id": "g9b_common_alu_ctcf_blood_eqtl_v1",
        ...
        _progress(f"verdict={decision['verdict']} reason={decision['reason']}")
        return 0

    _progress("PRIMARY QTD000356 cases…")
    case_arm = _query_arm(cases, PRIMARY_DATASET)
    _progress("PRIMARY QTD000356 controls…")
    ...
    if decision["verdict"] == "PASS":
        _progress("REPLICATION QTD000373…")
