#!/usr/bin/env python3
"""Download C-D1 primary inputs (ENCODE bedpe + UCSC rmsk)."""
...
USER_AGENT = "DNA-Ladder-C-D1/1.0 (desk; public ENCODE/UCSC only)"
ENCODE = "https://www.encodeproject.org"

FILES = {
    "ENCFF511QFN": {
        "role": "pol2_chia_pet_loops",
...
def download(url: str, dest: Path, expected_md5: str | None) -> dict:
    ...
    if dest.exists() and dest.stat().st_size > 0:
        ...
        if expected_md5 and digests["md5"] != expected_md5:
            ...
            return {"status": "cached", "path": str(dest), "size_bytes": dest.stat().st_size, **digests, "url": url}
    ...
    digests = hash_file(dest)
    if expected_md5 and digests["md5"] != expected_md5:
        return {"status": "md5_mismatch", "path": str(dest), "expected_md5": expected_md5, **digests, "url": url}
    return {"status": "downloaded", "path": str(dest), "size_bytes": dest.stat().st_size, **digests, "url": url}
